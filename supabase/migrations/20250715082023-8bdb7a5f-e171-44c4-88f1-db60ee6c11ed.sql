-- Phase 3: Legacy Data Migration (Corrected)
-- This phase migrates existing cart_items and order_items to use the new variant system
-- Skip column creation as they already exist

-- Step 1: Backup existing product_id values (only if not already done)
...
-- Step 2: Create function to map legacy product IDs to new UUIDs
...
-- Step 3: Create function to get default variant for a product
...
-- Step 4: Update cart_items with variant_id
UPDATE public.cart_items 
...
-- Step 5: Update order_items with variant_id  
UPDATE public.order_items 
SET variant_id = public.get_default_variant_for_product(
  public.map_legacy_product_id_to_uuid(legacy_product_id)
)
...
-- Step 6: Create validation function for Phase 3
...
-- Step 7: Create function to get migration status
